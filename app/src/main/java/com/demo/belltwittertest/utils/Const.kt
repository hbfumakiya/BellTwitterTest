package com.demo.belltwittertest.utils

/**
 * Created by Hardik on 2019-10-12.
 */
object Const {

    const val TWITTER_CONSUMER_API_KEY= "Wl5BJqzjHh30yPkH1G1N5hcJd"
    const val TWITTER_CONSUMER_API_SECRET_KEY= "MzJ1rdM3YZoSEUeiLxsmhHzZWYiCRAVyRb47ZkaNBshgPCvPuN"

    const val TWITTER_ACCESS_TOKEN_KEY= "1097662795-kewmBipAh5BFPwQYZlUJNa4axCqg4OHrNzuxEpN"
    const val TWITTER_ACCESS_TOKEN_SECRET_KEY= "ZA1eQsjcAuBpEhUX8MOGpwbXFPzlxpDrUJVRv7rWObe3B"

    const val DEFAULT_RADIUS=10000.0 // 5kms
    const val DEFAULT_ZOOM_SCALE=16F

}